...
- Name: filter-drawer.spec.ts >> Filter Drawer (mobile) >> condition chip toggles on/off
- Location: e2e\filter-drawer.spec.ts:128:7
...
Locator:  getByRole('button', { name: 'ใหม่ในกล่อง' })
Expected: "600"
Received: "400"
Timeout:  5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: 'ใหม่ในกล่อง' })
    9 × locator resolved to <button>ใหม่ในกล่อง</button>
...
- generic [ref=e1]:
  ...
  - main [ref=e58]:
    ...
    - button "ตัวกรอง" [ref=e114] [cursor=pointer]:
      - img [ref=e115]
      - text: ตัวกรอง
  ...
  - generic [ref=e127]:
    ...
    - generic [ref=e133]:
      ...
      - generic [ref=e154]:
        - button "ใหม่ในกล่อง" [active] [ref=e155] [cursor=pointer]
...
```ts
  32  |     await expect(page.getByText('หมวดหมู่')).toBeVisible();
  33  |     await expect(page.getByText('ช่วงราคา')).toBeVisible();
  34  |     await expect(page.getByText('สภาพสินค้า')).toBeVisible();
  35  |     await expect(page.getByText('วิธีรับสินค้า')).toBeVisible();
  36  |   });
  37  | 
  38  |   test('drawer shows category chips including "ทั้งหมด"', async ({ page }) => {
  39  |     await page.getByRole('button', { name: /ตัวกรอง/ }).click();
  40  |     await expect(page.getByRole('button', { name: 'ทั้งหมด' }).first()).toBeVisible();
